...
`frontend/src/ws.ts` 是浏览器侧的**唯一**出口/入口（§49）。
它踩过的四个坑都写在这里，因为它们**都不会抛错**，只会让界面静默地不对。

### 5.1.1 前端不实现 ACK 语义

后端在收到 `robot_command` 之后要**先 step、再回帧**（见上一节）。
所以"命令被接受"与"命令已生效"之间隔着一次物理推进。

若前端把"发出去了"当作"生效了"，就会在滑块刚拖动时显示新位置 ——
**而那个位置是前端自己编的，不是后端的状态**。§49 的唯一闭环当场作废。

⇒ 前端的规则：`sendCommand` 只表示"socket 接受了这帧"，
   界面上的**权威**数字一律等回帧。

### 5.1.2 不要用 `performance.getEntriesByType('resource')` 探测 WebSocket

WebSocket 握手**不进 resource timing**（它不是 fetch/XHR 资源）。
那条判据**永远是假**，却写得很像"连上了"。

⇒ 判据要落在 DOM 上（面板里的连接状态字/数字），或者直接 `eval` 页面里
   那个真实 `WebSocket` 实例的 `readyState`。
   见 `tools/e2e_browser_check.mjs` 第 ⑧ 节。

### 5.1.3 `useSyncExternalStore` 的快照必须引用稳定、且**整体**换新

```text
① getSnapshot() 每次现造新对象
   ⇒ React 用 Object.is 比较 ⇒ 认为"每次都变了" ⇒ 无限重渲染

② 拆成 N 个 useSyncExternalStore（state / lastState / … 各一个）
   ⇒ notify() 一次通知全部，但 React 对"快照没变"的那个跳过重渲染
   ⇒ 字段之间的一致性只能靠运气
```
...
```text
链路面板显示「未连接」，但关节面板 / 滑块 / 3D 画面全都正常渲染。
```

看起来像"WebSocket 没连上"，实际连接是好的 ——
只是"连接状态"与"状态帧"这两个订阅器各自收敛到了不同的时刻。

⇒ 正确做法（已在 `ws.ts` 落地）：快照是**一个**对象，
   **只在 `notify()` 里重建一次**，订阅者只有**一个**。

### 5.1.4 断线不清空 `lastState`

清空会让界面在断网的瞬间显示"所有关节 = 0"——
用户会以为机器人自己回零了。保留最后已知值 + 把连接状态标红才是诚实的。

同理，`error` 帧**不代表连接已断**，不触发重连；
只有 `close` 才排重连，且退避必须**有上限**。
