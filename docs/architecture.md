...
### 10.5 ⚠️ 跑测试的**退出码**不是"测试结果"

一律写 `check("pytest ...", proc.returncode == 0, ...)`。这个写法在
本机沙箱下产出过一次**大规模假回归**（2026-09-15，实测）：
...
**测试全过**（`[100%]` + 一整串通过点），却判 FAIL，并且
Phase 1~5 的上游复查全部连坐（Phase 7 一度报 39/50）。代码一行没错。

**根因**：沙箱有"批量删除守卫"（单 turn 内删除 >5000 个文件被拦）。
pytest 每跑一次都在 `%TEMP%/pytest-of-zc110/` 建一棵临时树并在结束时清理；
一条清单会连跑 7 次 pytest ⇒ 累计删除量越过阈值
⇒ **清理被拦 ⇒ 退出码非 0**。

⇒ 规则：**判据必须量它声称要量的东西。** 这里要量的是"测试有没有过"，
不是"进程退出码"（后者还掺了"临时目录清理有没有被放行"）。

修法是读 pytest **自己的摘要**，两级判词（`tools/_pytest_verdict.py`）：
...
⚠️ 第 ② 级的三个条件必须**同时**成立，否则恒真：
只看"没有 failed"的话，**空输出**也会判过。元测试（放在 Phase 1，
因为它是所有清单共用的判据基础）覆盖 10 个场景，含
"全过+沙箱噪声粘尾必须判过"与"空输出必须判不过"。

**同类问题**：`.ts` runner 与 `run_python` 的检查同样别把退出码当唯一判据。
...
| 认证 / 多用户 | 不涉及 |
| `transport/` 抽象 | `Transport` Protocol 已就位（§48），但**没有任何实现与调用方** |
| 传输层真机对接 | 串口 / CAN / USB / TCP 一律不做；`receive()` 的返回约定**尚未被任何代码验证** |
| 拖拽示教 / 路径编辑 | 前端只有可视化 + 滑块，无轨迹录制 |
| 碰撞可视化 / 逆解可视化 | 不做 |

### 11.1 前端的两条"静默全黑"约束（踩过，写进这里防复发）

这两条**不是**"不做"，而是"做了会被坑"——它们让 488 个 pytest 全绿、
REST 全 200、WS 探针 25/25 的同时，**浏览器里是一片全黑**。
详细成因与源码链条见 §10.2 / §10.3。

| 约束 | 违反后的症状 |
|---|---|
| r3f 元素上的属性名**不得含 `-`**（禁用 `data-*`） | 整棵场景树被 ErrorBoundary 捕获重挂载 ⇒ `#root` 为空 ⇒ **全黑，且报错栈里看不到业务代码** |
| 读 WebGL 像素必须在 `requestAnimationFrame` **之内** | 帧外 `readPixels` 恒返回全 0（`gl.getError()` 仍是 `0`） ⇒ 判据误报"页面全黑"，而同一次运行的截图明明有内容 |
