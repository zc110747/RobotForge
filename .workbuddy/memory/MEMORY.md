...
**§49 唯一闭环（前端唯一的姿态来源）**：
`Frontend → WebSocket → RobotCommand → Runtime → MuJoCoBackend → MuJoCo
→ RobotState → WebSocket → Frontend`。**禁止** `Frontend → 直接改 Three.js`。

## 铁律：Phase Gate
> **当前 Phase 未通过：不得进入下一 Phase。**
每 Phase 收尾跑 `tools/accept_phaseN.py` 全绿，数字必须显式报出。

## 当前基线（2026-09-15，Phase 8 收口 + 前端闭环）
```text
pytest -q → 488 passed ｜ pytest packages/mini_arm/tests → 34 passed
accept_phase1 → 34/34 ｜ 2 → 56/56 ｜ 3 → 47/47 ｜ 4 → 65/65 ｜ 5 → 67/67
accept_phase7 → 50/50 ｜ accept_phase8 → 65/65
tools/probe_ws_live.py → 25/25 PASS
frontend: tsc 0 错 ｜ vite build 628 modules
  ws.check / ghost.check / predictor.check / kinematics.check  → 各 PASS + 自检 5/5
  semantics.check 121 项 ｜ geometry.check 43 项 ｜ render.check 124 项
  e2e_browser_check.mjs → 25 PASS / 0 FAIL / 0 SKIP
```
⚠️ **计数口径**：Phase 3+ 的清单各自跑一遍全部上游，那趟以「上游无回归」
子项出现，不计入本 Phase 的分子/分母。旧记忆里的 `7 → 84/84`、`8 → 96/96`
...
前端闭环        命令是**持续目标**（不是一次性脉冲）
                前端**不**夹紧命令（限幅是后端职责；前端夹会丢失"超限"信息）
                快照是**一个**引用稳定对象，在 notify() 里**先重建、再通知**
                可重复开启的会话 + 异步回调 ⇒ 必须用**代际号**，布尔不够
                （前两条的完整案例见 CONTRACTS.md §5.1）
...
6. **门槛必须落在被测对象能达成的范围内**。实测：单步限幅 0.35 却断言位移 `> 0.5`
   ⇒ 永远红，且失败信息误导成"命令被覆盖"。
7. **同类：短名字做裸子串匹配必误报**。`obj` 命中 `object`、`step` 命中 spec 强制
   的 `def step()` 接口。要词边界匹配。**绝不能为了清单变绿去改契约本身**。
   也不要惩罚"把话说清楚"：判据逼迫你把错误提示写模糊 ⇒ 先怀疑判据。
...
- 正则批量改写要锚定"哪个函数的返回值"，不能只锚定"变量叫什么"
  （**改名不改语法 ⇒ `ast.parse` 发现不了，只有真跑才炸**）。
- 扰动自检先实测两件事：(a) 该"扰动"语义上真是扰动吗？(b) 被测函数真读这个量吗？
- 子进程结果解析用 `json.JSONDecoder().raw_decode()`，**不要**字符串切片。

### 前端自检脚本（ts）
```text
✅ node --experimental-strip-types src/xxx.check.ts
❌ node src/xxx.check.ts   ← ERR_UNSUPPORTED_TYPESCRIPT_SYNTAX，
                            但**退出码 0**、只有 9 行输出 ⇒ 极易误判成 PASS
```
`--experimental-strip-types` 硬限制：不支持**参数属性**（`constructor(readonly x)`）、
不支持 **JSX**（`.tsx` 必须先用 esbuild 打 `.mjs`）。

### e2e（`tools/e2e_browser_check.mjs`）
- **卡死 ≠ 产品问题**：两个实例共用 debug 端口会互相冲突（曾卡 10 min）。
  判据：独立探 `http://127.0.0.1:<port>/json/version`，`ECONNREFUSED` = 端口被占。
  `TaskStop` 旧实例时它 `finally` 的 `child.kill()` 会连带杀掉浏览器。
- **跨段状态必须显式恢复 + 断言**（例：第 ⑥ 段点了「隐藏面板」没点回来，
  第 ⑧ 段全部查询落空，失败信息却像渲染 bug）。
- 判据要读**页面自己的 DOM**；WebSocket 握手**不进** resource timing。
- React 测试的隐蔽前提（先装 `document` 再 import / 原生 value setter /
  StrictMode effect 交错）见 CONTRACTS.md §5.1。
...
bash x.sh                      ✗ 解析到 System32\bash.exe（WSL 存根）被拦，
                                 症状是 **exit=0 + 乱码**且无 trace
                                 → 用 `/usr/bin/bash x.sh` 或 `sh`
npm shim                       ✗ uname 判成 Linux → 调 wsl.exe 被拦 → tools/npm.sh
npm --prefix <dir> install     ✗ 不生效 → **先 cd 到目标目录**再跑
vite/tsc 也要先 cd 到 frontend/  ✗ 从仓库根跑 ./node_modules/.bin/* 找不到
node versions/current          ✗ **不是目录**，是 9 字节文件（内容=版本名）
heredoc 写非 ASCII / # 内容     ✗ 实测会**前置**而非追加 → 用 Write 工具
PowerShell 工具输出不回显       ✗ 写 $env:TEMP\*.txt 再用 Read 读
```
- 本工程 python 用 **`./.venv/Scripts/python.exe`**（不是 managed python —— 那个
  收集阶段就 ERROR）。pytest **不要**写 `| tail`，摘要会丢。
...
## git 卫生（别改回去）
- `origin = https://github.com/zc110747/RobotForge.git`；分支 `develop`。
- **`node_modules/` 与 `.venv/` 必须忽略**（曾有 8211 个文件来自
  ...
  `frontend/src/**/__*.check.mjs`（esbuild 中间产物）。
