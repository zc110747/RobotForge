...
## 项目定位
按 spec `RobotForge v0.1 完整统一开发提示词`（76 节）从零构建。
链路：**Robot Package → Loader → RobotModel → RobotRuntime → Backend**。
`RobotModel` 是**唯一规范内部表示**；只允许原生 MJCF；Core/Frontend
**不得出现机器人型号分支**（§69 规则 2）。

坐标：右手系，`+X 前 / +Y 左 / +Z 上`，`X × Y = Z`；SI 单位；
四元数 `[x,y,z,w]`。**§41：不得在 FK / IK 里为 Three.js 修改坐标** ——
唯一转换点是 `frontend/src/viewer/coordinateAdapter.ts`。

## 铁律：Phase Gate
> **当前 Phase 未通过：不得进入下一 Phase。**

每个 Phase 收尾必须跑 `tools/accept_phaseN.py` 全绿，且数字要显式报出。
...
tools/accept_phase7.py           → 84/84（内含全部上游，约 12 min）
tools/accept_phase8.py           → 96/96（内含全部上游，约 12 min）
```
Phase 0 ✅ / 1 ✅ / 2 ✅ / 3 ✅ / 4 ✅ / 5 ✅ / 7 ✅ / 8 ✅
（**spec 无 Phase 6**，§64 直接是 Phase 7）

## 扩展点契约（Phase 7 / 8 决定，**别改回去**）
```text
...
第三方：third_party 或包内
```
- 依赖方向只能 **kinematics → model**；`model` 不得 import `kinematics`。
...
kinematics → model          （kinematics 是纯数学）
model      ← 不得 import kinematics / runtime
```
- Core `kinematics` 不得 import `mujoco` / `three` / `fastapi` / `runtime`。
- `runtime` 不得 import `fastapi` / `mujoco` / `starlette`（纯编排层）。
- `runtime/backend.py` 不得 import `api.registry`（Backend 只吃 RobotModel）。
- 包内模块按**绝对文件路径**加载（`importlib.util.spec_from_file_location`）
  且**必须** `sys.modules[name] = mod`（Python 3.13 `@dataclass` 需反查模块）。
- CLI 等入口一律从 `manifest.kinematics.<tag>.entry` **动态加载**，
  不得硬编码包路径或型号名。

## Runtime 层契约（Phase 4 决定，**别改回去**）
```text
...
往返/一致性这类验收，**不要写 `Δ < TOL`**，要写"先由几何量预测类别，
再断言实测吻合，且不允许出现无法解释的类别"。

理由：容差阈值必须覆盖最坏情况（mini_arm 的 2R 双解里最坏是 π），
于是"1° 的系统性错误"被放过。分类要求每个样本都能被机制解释。

mini_arm 的往返定律（全网格 11³ 双向验证，交叉项为 0）：
```text
...
## 判据写法铁律二：写**不变量**，不写"我期望的终值"
任何有**串联约束**的路径（限幅 → 限速 → 目标），判据必须落在
**各自独立可观测的点**上，不能只看最终状态。

Phase 4 实测三例（引擎全对，判据全错）：
```text
...
## 判据写法铁律三：判据不能与"被验对象的语义"自相矛盾
写完一条断言先问：**"这个表达式在什么情况下会是假？"**
答不出（或答案与业务无关）⇒ 它恒真/恒假。Phase 5 抓到两条恒假断言：
...
## 判据写法铁律四：物理量容差里要**命名**残余误差来源
纯运动学断言（FK 互证、四元数换算）用**机器精度**（1e-12 ~ 1e-9）；
物理量用 5e-3，且注释写明"这个容差在承担什么"：
```text
...
cp x /tmp/x                    ✗ Permission denied → 放 .workbuddy/
grep -oE '...'                 ✗ `-u系统找不到指定的文件` → 用 Grep 工具
curl -o /dev/null -w 连跑      ✗ Exit 23 中断链 → 逐条跑或落文件
bash x.sh                      ✗ System32\bash.exe（WSL 启动器）→ /usr/bin/bash
